...
class Prepare_Mercedes(Prepare_Data):
    def __init__(self,interval_type='monthly'):
        super(Prepare_Mercedes, self).__init__()
        self.interval_type=interval_type
        self.import_examples(['./raw_data/Production_list201603-201609.xlsx', './raw_data/Repair_list.xlsx'])
        self.create_feature_vehicle()
        self.find_interval()
        self.find_failures()
        self._fillup_registration_date()
        self.create_feature_samples()
        self.create_feature_failure_rate()

    def import_examples(self,data_dir):
        self.samples = self._read_excel(data_dir[0],
                                        parse_dates=['Production date','Initial registration date'],
                                        date_parser=lambda x: pd.datetime.strptime(str(x),"%m/%d/%Y"))
        self.failures = self._read_excel(data_dir[1],
    ...
    def find_interval(self):
        ...
        self.samples['Interval']=list(map(lambda x:x.days,upto_date-self.samples['Initial registration date']))
    ...
    def _fillup_registration_date(self):
        pass
