...
class UnfoldBatchToSpatial(tf.keras.layers.Layer):
    ...
    def call(self, inputs, batch_size=None):
        ...
        target = tf.concat([tf.reshape(batch_size, [1]), tf.constant(self.spatial_shape), shape[1:]], axis=0)
        restored = tf.reshape(inputs, target)
        spatial_rank = len(self.spatial_shape)
        rank = inputs.shape.rank
        if rank is None:
        ...
        return tf.transpose(
            restored,
            [0, spatial_rank + 1]
            + list(range(1, spatial_rank + 1))
            + list(range(spatial_rank + 2, spatial_rank + rank)),
        )
...
class SpatialBatchTransform:
    ...
    def __init__(self, strategy: str):
        if strategy not in {"per_node", "flatten"}:
    ...
    def apply(self, batch: TimeSeriesBatch):
        ...
        set_size = batch.structure.set_size
        values = {}
        for field in fields(batch):
            name, value = field.name, getattr(batch, field.name)
            if name in {"structure", "metadata"} or value is None:
                continue
            ...
            elif name in {"padding_mask", "labels"} and self.strategy == "per_node":
    ...
    def _temporal(self, value, spatial_rank, set_size):
        if self.strategy == "per_node":
            ...
            return FoldSpatialToBatch()(value)
        if value.shape.rank == 3:
            return value
        shape = tf.shape(value)
        return tf.reshape(value, [shape[0], shape[1], -1])
    ...
    def _static(self, value, spatial_rank, set_size):
        ...
        if value.shape.rank == 2:
            return value
        shape = tf.shape(value)
        return tf.reshape(value, [shape[0], -1])

    def _restore(self, original: TimeSeriesBatch) -> Callable[[tf.Tensor], tf.Tensor]:
        if self.strategy == "flatten":
            return lambda value: value
        unfold = UnfoldBatchToSpatial(original.spatial_shape)
        return lambda value: None if value is None else unfold(value, batch_size=original.batch_size)
