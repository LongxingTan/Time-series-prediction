...
from ..layers.transformer_layer import CustomAttention
...
class Encoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self.rnn_type.lower() == 'gru':
            self.rnn = GRU(units=self.rnn_size, activation='tanh', return_state=True, return_sequences=True,
            ...
            self.rnn2 = LSTM(units=self.rnn_size, activation='tanh', return_state=True, return_sequences=True,
                             dropout=self.rnn_dropout)
...
class RNNBaseline(object):
    ...
    def __init__(self, predict_sequence_length=3, custom_model_params=None) -> None:
        ...
        self.params = params
        self.predict_sequence_length = predict_sequence_length
        self.rnn = GRU(units=params['rnn_size'], activation='tanh', return_state=True, return_sequences=True,
                       dropout=rnn_dropout)
