...
from .features import multi_step_y, transform2_lagged_feature
...
class SineData(DataSet):
    ...
    def load_data(self, data_dir=None, *args):
        ...
        for _ in range(n_examples):
            ...
            sig1 = np.sin(np.linspace(rand, 3.0 * np.pi + rand, sequence_length + predict_sequence_length))
            sig2 = np.cos(np.linspace(rand, 3.0 * np.pi + rand, sequence_length + predict_sequence_length))
